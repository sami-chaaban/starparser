...
* **```--multiply```** *```column-name/value```* : Multiply all values of a column by a value. The argument to pass is column/value (e.g. \_rlnHelicalTrackLength/0.25). The result is written to a new star file (default output.star, or specified with ```--o```).
